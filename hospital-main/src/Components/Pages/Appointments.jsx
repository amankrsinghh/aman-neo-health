...
function Appointments() {
    ...
    async function getAppointmentData() {
        setLoading(true)
        try {
            const result = await getSecureApiData(`appointment/hospital/${userId}?page=${currentPage}&limit=10&search=${search}&startDate=${startDate}&endDate=${endDate}&statuses=${status}&doctorId=${doctorId}`)
            if (result.success) {
